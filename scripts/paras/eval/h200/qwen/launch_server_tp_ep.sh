#!/bin/bash
# launch_server — Qwen3-235B-A22B-Instruct-2507, TP/EP (TP attention + EP experts via AllReduce), H200.
# Experts are partitioned across ranks (--ep-size N) with no DeepEP; each rank runs its
# local experts on the full token batch with masking, combined via full-hidden AllReduce.
# Requires the DeepGEMM-EP gating fix (commit 637a0394) so BF16 AllReduce-EP falls back to
# the Triton MoE runner instead of tripping the fp8 masked GEMM on SM90.
