...
paras_launch_setup_dp_ep() {
    ...
    if [ "$ENABLE_PARAS" = "1" ]; then
        ...
        : "${PARAS_INTRA_NODE_WEIGHT_TRANSFER_METHOD:=peer_access}"
        : "${PARAS_KV_TRANSFER_METHOD:=peer_access}"
        : "${PARAS_DISABLE_PEER_ACCESS:=0}"
        export SGLANG_ATTN_MAX_BS PARAS_INTRA_NODE_WEIGHT_TRANSFER_METHOD PARAS_KV_TRANSFER_METHOD PARAS_DISABLE_PEER_ACCESS
        PARAS_FLAGS=(
            --enable-paras-moe
