#!/bin/bash
# 16-GPU PP4×TP4 skew comparison: 4 profiles × 1 load level
# Cluster: aisys-303, 4 nodes × 4 A6000 = 16 GPUs
# Usage: bash run_pptp16_skew_comparison.sh <dist_init_addr> <worker1_host> <worker2_host> <worker3_host>
set -e
...
launch_pptp16() {
    ...
    tmux new-session -d -s sglang-head \
        "bash ${SCRIPT_DIR}/launch_head_pptp_record.sh \
...
echo "================================================================"
echo "PP4×TP4 Skew Comparison: ${#PROFILES[@]} profiles"
echo "Params: ${NUM_PROMPTS} prompts, rate=${REQUEST_RATE}, input=${INPUT_LEN}, output=${OUTPUT_LEN}"
echo "Cluster: aisys-303, 4 nodes × 4 GPUs = 16× A6000"
