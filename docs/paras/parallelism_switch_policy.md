...
2. `scheduler_paras_mixin.py`: on `paras_configure_tp`, repoint `sampler.tp_sync_group` to `self.paras_tp_group.device_group` (the full 8-rank TP group) and set `sampler.force_sync_token_ids = True`. On `paras_configure_ep`, restore `sampler.tp_sync_group` to `self.paras_ep_attn_tp_group.device_group` (the EP-mode single-rank attn-tp group) and clear the force flag.
